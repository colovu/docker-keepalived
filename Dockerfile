...
# 安装临时使用的软件包及依赖项。相关软件包在镜像创建完后时，会被清理
	...
	apt update; \
#	apt upgrade -y; \
	apt install -y --no-install-recommends ${fetchDeps} ${appDeps}; \
	\
	\
	\
...
# 源码编译方式安装: 编译后将原始配置文件拷贝至 ${APP_DEF_DIR} 中
	...
	cd ${APP_SRC}; \
#	sed -i -e "s/^  PID_DIR=\/var/  PID_DIR=\/var\/run\/keepalived/g" configure; \
	./configure \
#		--with-run-dir=/var \
#		--runstatedir=/var/run/keepalived \
		--disable-dynamic-linking; \
	make -j "$(nproc)"; \
	make install; \
	cp -rf /usr/local/etc/keepalived/* ${APP_DEF_DIR}/; \
	cd /; \
